...
try:
    ...
    if LMNT_API_KEY:
        print("✅ LMNT API key loaded successfully")
        os.environ["LMNT_API_KEY"] = LMNT_API_KEY  # Ensure it's set for lmnt library
    else:
        print("⚠️  LMNT_API_KEY not found in environment variables")
except Exception as e:
    print(f"⚠️  Failed to load LMNT API key: {e}")

# Try to import LMNT
try:
    from lmnt.api import Speech
    LMNT_AVAILABLE = True
    print("✅ LMNT library imported successfully")
except ImportError:
    LMNT_AVAILABLE = False
    print("⚠️  LMNT library not installed. Install with: pip install lmnt")
...
class AudioNarrationService:
    ...
    def __init__(self, output_dir: str = None):
        ...
        self.lmnt_available = LMNT_AVAILABLE and LMNT_API_KEY
    
    def generate_narration_for_scene(self, scene_block: AnsciSceneBlock, scene_name: str, target_duration: float = None) -> Optional[str]:
        """
        ...
        if not self.lmnt_available:
            print("⚠️  LMNT API not available, using fallback audio generation")
        ...
        try:
            ...
            if target_duration:
                print(f"   🎬 Animation duration: {target_duration:.1f}s (audio will match)")
            
            # Generate audio using LMNT TTS or fallback
            audio_path = self._generate_lmnt_audio(enhanced_transcript, scene_name, target_duration)
    ...
    def generate_narrations_for_animation(self, animation: AnsciAnimation, video_paths: List[str] = None) -> List[str]:
        """
    ...
    def merge_audio_with_video(self, video_path: str, audio_path: str, output_name: str = None) -> str:
        """
        ...
        cmd = [
            ...
            "-t", str(video_duration),  # Use video duration as master
            "-af", f"apad=whole_dur={video_duration}",  # Pad audio to match video duration
            str(output_path)
        ]
        ...
        try:
            ...
            print(f"   ✅ Final duration: {final_duration:.1f}s (matches video: {abs(final_duration - video_duration) < 0.5})")
    ...
    def create_complete_audiovisual_animation(self, animation: AnsciAnimation, video_paths: List[str]) -> List[str]:
        """
    ...
    def _adjust_transcript_for_animation_duration(self, transcript: str, description: str, animation_duration: float = None) -> str:
        """
        ...
        print(f"   📊 Speech analysis: {len(words)} words, natural duration: {natural_duration:.1f}s")
        ...
        if animation_duration > natural_duration * 1.5:
            ...
            enhanced = self._expand_transcript_for_longer_animation(transcript, animation_duration)
            ...
            enhanced = self._condense_transcript_for_shorter_animation(transcript, animation_duration)
        else:
    ...
    def _expand_transcript_for_longer_animation(self, transcript: str, target_duration: float) -> str:
    ...
    def _condense_transcript_for_shorter_animation(self, transcript: str, target_duration: float) -> str:
        """Condense transcript for shorter animations"""
        # Remove unnecessary words and phrases
        condensed = transcript
        
        # Remove filler words
        filler_words = ['basically', 'essentially', 'actually', 'really', 'very', 'quite']
    ...
    def _generate_lmnt_audio(self, text: str, scene_name: str, target_duration: float = None) -> Optional[str]:
        ...
        try:
            if not self.lmnt_available:
                print("⚠️  LMNT not available, falling back to system TTS")
    ...
    def _adjust_audio_duration_to_animation(self, audio_path: Path, scene_name: str, target_duration: float = None) -> Optional[str]:
        ...
        try:
            ...
            if abs(current_duration - target_duration) < 0.5:
                ...
                audio_path.rename(final_path)
                print(f"   ✅ Audio duration matches animation: {current_duration:.1f}s")
                return str(final_path)
            
            elif current_duration < target_duration:
                # Audio is shorter than animation - add silence at the end
                silence_duration = target_duration - current_duration
                print(f"   🔇 Adding {silence_duration:.1f}s silence to match animation")
                
                cmd = [
                    "ffmpeg", "-y",
                    "-i", str(audio_path),
                    "-f", "lavfi", "-i", f"anullsrc=channel_layout=stereo:sample_rate=48000",
                    "-filter_complex", f"[0:a][1:a]concat=n=2:v=0:a=1",
    ...
    def _fallback_system_tts(self, text: str, scene_name: str, target_duration: float = None) -> Optional[str]:
        ...
        try:
            ...
            cmd = [
                ...
                "-t", str(duration),
                "-c:a", "mp3",
                "-metadata", f"title=Narration: {text[:50]}...",
                "-metadata", f"artist=AnSci Animation System",
                str(audio_path)
            ]
            
            subprocess.run(cmd, capture_output=True, check=True)
            print(f"✅ Generated silent audio placeholder: {audio_path} ({duration:.1f}s)")
...
def create_audiovisual_animation(animation: AnsciAnimation, video_paths: List[str], output_dir: str = None) -> List[str]:
    """
...
def create_audiovisual_animation_with_embedded_audio(animation: AnsciAnimation, output_dir: str = None) -> AnsciAnimation:
    """
